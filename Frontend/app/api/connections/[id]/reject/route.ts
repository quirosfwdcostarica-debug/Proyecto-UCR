...
// PUT /api/connections/[id]/reject — rechazar solicitud pendiente recibida
  ...
  try {
    ...
    if (match?.initiated_by) {
      await supabaseAdmin.from("NOTIFICATIONS").insert({
        ...
        message: "Rechazaron tu solicitud de conexión.",
        type: "match_rejected",
        read: false,
    ...
    console.error("[PUT /api/connections/:id/reject]", error);
    return NextResponse.json({ message: "Error al rechazar la conexión" }, { status: 500 });
  }
}

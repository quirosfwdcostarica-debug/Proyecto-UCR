        ...
        <div className="bg-white border border-slate-200 p-5 rounded-xl shadow-sm">
          <div className="flex justify-between items-center mb-4">
            ...
            <button 
              ...
              className="text-xs text-ucr-celeste-medium hover:underline font-semibold"
            >
              Limpiar
            </button>
          </div>
          ...
          <div className="space-y-5">
            {/* Career Selector */}
            <div>
              <label className="block text-xs font-semibold text-slate-500 mb-2 uppercase">Carrera</label>
              <select 
                value={selectedCarrera}
                onChange={(e) => setSelectedCarrera(e.target.value)}
                className="w-full h-9 border border-slate-200 rounded text-sm text-slate-700 px-2 outline-none focus:border-ucr-celeste-medium"
              >
                {careers.map((car, idx) => (
                ...
                {industries.map((ind) => (
                  <label key={ind} className="flex items-center gap-2 cursor-pointer">
                    <input 
                      ...
                      className="rounded border-slate-300 text-ucr-celeste-medium focus:ring-ucr-celeste-medium" 
                    />
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-6">
          ...
          <div className="flex items-center gap-4 bg-white border border-slate-200 rounded-md p-1 self-start sm:self-auto shadow-sm">
            <button 
              ...
              className={`p-1.5 rounded transition-colors ${viewMode === "grid" ? "bg-ucr-celeste-medium text-white" : "text-slate-400 hover:text-slate-600"}`}
            >
              <LayoutGrid className="w-4 h-4" />
            </button>
            <button 
              onClick={() => setViewMode("list")}
              className={`p-1.5 rounded transition-colors ${viewMode === "list" ? "bg-ucr-celeste-medium text-white" : "text-slate-400 hover:text-slate-600"}`}
            >
        ...
        {filteredAlumni.length === 0 ? (
          ...
          <div className={viewMode === "grid" 
            ...
            {filteredAlumni.map((alumni) => {
              ...
              if (viewMode === "list") {
                return (
                  <Card key={alumni.user_id} className="p-4 border-slate-200 hover:border-ucr-celeste-medium/30 transition-all shadow-sm bg-white flex flex-col sm:flex-row items-center gap-4">
                    ...
                    <div className="flex gap-2 w-full sm:w-auto mt-2 sm:mt-0 shrink-0">
                      <Link href={`/perfil/${alumni.user_id}`} className="flex-1 sm:flex-none">
                        <Button className="w-full bg-ucr-celeste-medium hover:bg-ucr-celeste-medium/90 text-white py-2 px-4 text-xs font-semibold h-9">
              ...
              return (
                <Card key={alumni.user_id} className="overflow-hidden border-slate-200 hover:border-ucr-celeste-medium/30 transition-all shadow-sm hover:shadow-md flex flex-col bg-white">
                      ...
                      {tags.slice(0, 3).map((tag, j) => (
                        <span 
                          ...
                          className={`px-2 py-0.5 rounded text-[8px] font-bold tracking-wide ${
                            j === 0 ? 'bg-ucr-celeste-tint text-ucr-celeste-medium' : 'bg-slate-100 text-slate-600'
                          }`}
                        >
                          {tag}
                        </span>
